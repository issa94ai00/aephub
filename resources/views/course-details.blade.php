...
@section('content')
        <div class="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-14">
            <section class="reveal site-panel">
                ...
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div class="max-w-3xl">
                        ...
                        <h1 class="mt-3 text-2xl font-bold tracking-tight text-slate-950 dark:text-slate-50 sm:text-3xl">{{ $course->localized_title }}</h1>
                    ...
                    <div class="rounded-2xl border border-emerald-200/60 bg-gradient-to-b from-emerald-50/95 to-emerald-100/40 px-5 py-4 text-center shadow-inner dark:border-emerald-800/45 dark:from-emerald-950/40 dark:to-emerald-950/20">
                        <div class="text-xs font-semibold text-slate-700 dark:text-slate-400">{{ __('site.course.price') }}</div>
                ...
                <div class="mt-8 grid gap-3 sm:grid-cols-3">
                    <div class="rounded-2xl border border-slate-200/70 bg-slate-50/90 p-4 shadow-inner dark:border-slate-700/60 dark:bg-slate-800/80">
                        <div class="text-xs font-semibold text-slate-700 dark:text-slate-400">{{ __('site.course.instructor') }}</div>
                        <div class="mt-1 text-sm font-bold text-slate-950 dark:text-slate-50">{{ $course->teacher->name ?? __('site.home.not_set') }}</div>
                    </div>
                    <div class="rounded-2xl border border-slate-200/70 bg-slate-50/90 p-4 shadow-inner dark:border-slate-700/60 dark:bg-slate-800/80">
                        <div class="text-xs font-semibold text-slate-700 dark:text-slate-400">{{ __('site.course.video_count') }}</div>
                        <div class="mt-1 text-sm font-bold text-slate-950 dark:text-slate-50">{{ $course->videos_count }}</div>
                    </div>
                    <div class="rounded-2xl border border-slate-200/70 bg-slate-50/90 p-4 shadow-inner dark:border-slate-700/60 dark:bg-slate-800/80">
                        <div class="text-xs font-semibold text-slate-700 dark:text-slate-400">{{ __('site.course.student_count') }}</div>
                        <div class="mt-1 text-sm font-bold text-slate-950 dark:text-slate-50">{{ $course->enrollments_count }}</div>
                    </div>
                </div>
            ...
            @if($relatedCourses->isNotEmpty())
                <section class="mt-10">
                    <h2 class="mb-5 text-xl font-bold tracking-tight text-slate-950 dark:text-slate-50">{{ __('site.course.related') }}</h2>
                        ...
                        @foreach($relatedCourses as $item)
                            <article class="reveal course-card">
                                ...
                                <div class="mt-4 flex items-center justify-between gap-3">
                                    <span class="text-xs font-medium text-slate-700 dark:text-slate-400">{{ __('site.home.videos_n', ['count' => $item->videos_count]) }}</span>
