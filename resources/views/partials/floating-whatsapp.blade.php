@php
    $settings = $site ?? [];
    $svc = app(\App\Services\SiteSettingsService::class);
    $waHref = $svc->resolveWhatsappHref($settings);
    $showFab = $svc->shouldShowFloatingWhatsapp($settings);
@endphp
@if($showFab)
    <a
        href="{{ $waHref }}"
        ...
        aria-label="{{ __('site.whatsapp.contact') }}"
        title="{{ __('site.whatsapp.title') }}"
    >
