...
@section('content')
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <a href="{{ route('admin.users.create') }}" class="admin-btn inline-flex items-center justify-center rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-900/25 hover:bg-emerald-500">
            + {{ __('admin.users.add_user') }}
        </a>
    </div>

    <div data-bulk-container>
        @php($hasStudents = $users->contains(fn ($u) => $u->role === 'student'))
        @if ($hasStudents)
            <div class="mb-3 flex flex-wrap items-center gap-3 text-xs">
                <label class="inline-flex cursor-pointer items-center gap-2 text-white/60">
                    <input type="checkbox" data-bulk-select-all class="accent-emerald-500" />
                    <span>{{ __('admin.users.select_all') }}</span>
                </label>
                <button type="submit" form="bulk-delete-form" data-bulk-button disabled
                        class="rounded-lg bg-rose-600 px-3 py-1.5 font-medium text-white opacity-40 disabled:cursor-not-allowed">
                    {{ __('admin.users.bulk_delete') }}
                </button>
                <span data-bulk-counter class="font-medium text-white/60"></span>
            </div>
        @endif

        <form id="bulk-delete-form" method="post" action="{{ route('admin.users.bulk-destroy') }}" onsubmit="return confirm(@json(__('admin.users.confirm_bulk_delete')));">
            @csrf
        </form>

        <div class="admin-table-wrap overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] ring-1 ring-white/5">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-white/10 text-sm" data-bulk-table>
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse ($users as $u)
                            <tr class="hover:bg-white/[0.02]">
                                @if ($hasStudents)
                                    <td class="px-4 py-3">
                                        @if ($u->role === 'student' && $u->id !== auth()->id())
                                            <input type="checkbox" name="user_ids[]" value="{{ $u->id }}" form="bulk-delete-form" data-bulk-checkbox class="accent-emerald-500" />
                                        @endif
                                    </td>
                                @endif
                                ...
                                <td class="px-4 py-3 text-end">
                                    <div class="flex flex-col items-end gap-1">
                                        <a href="{{ route('admin.users.edit', $u) }}" class="text-xs text-emerald-200 hover:underline">{{ __('admin.users.edit') }}</a>
                                        @if ($u->role === 'student' && $u->id !== auth()->id())
                                            <form method="post" action="{{ route('admin.users.destroy', $u) }}" onsubmit="return confirm(@json(__('admin.users.confirm_delete')));">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-xs text-rose-300 hover:underline">{{ __('admin.users.delete') }}</button>
                                            </form>
                                        @endif
                                        @if ($u->id !== auth()->id())
                                            @if (($u->status ?? 'active') === 'frozen')
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $hasStudents ? 9 : 8 }}" class="px-4 py-8 text-center text-white/55">{{ __('admin.users.empty') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
