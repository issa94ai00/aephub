@php
    ...
    /** @var \App\Models\StudyYear|null $year */
@endphp

<div class="mb-4">
    <a href="{{ route('admin.academics.universities.faculties.years.index', [$university, $faculty]) }}" class="text-xs text-emerald-200 hover:underline">← {{ app()->getLocale() === 'en' ? 'Back to years' : 'العودة للسنوات' }}</a>
</div>
...
<form method="post" action="{{ $action }}" class="max-w-2xl space-y-5">
    ...
    <div class="admin-card space-y-4 p-5">
        <div>
            <label class="block text-xs font-medium text-white/70">{{ app()->getLocale() === 'en' ? 'Year number' : 'رقم السنة' }}</label>
            <input type="number" name="year_number" min="1" step="1" value="{{ old('year_number', $year->year_number ?? 1) }}" required
        ...
        <div>
            ...
            <input name="name_en" value="{{ old('name_en', $year->name_en ?? '') }}"
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <a href="{{ route('admin.academics.universities.faculties.years.index', [$university, $faculty]) }}" class="rounded-xl border border-white/15 px-5 py-2.5 text-sm text-white/80 hover:bg-white/5">{{ app()->getLocale() === 'en' ? 'Cancel' : 'إلغاء' }}</a>
    </div>
</form>
