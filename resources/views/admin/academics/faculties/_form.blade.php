...
<div class="mb-4">
    <a href="{{ route('admin.academics.universities.faculties.index', $university) }}" class="text-xs text-emerald-200 hover:underline">← {{ app()->getLocale() === 'en' ? 'Back to faculties' : 'العودة للكليات' }}</a>
</div>
...
<form method="post" action="{{ $action }}" class="max-w-2xl space-y-5">
    ...
    <div class="admin-card space-y-4 p-5">
        ...
        <div>
            ...
            <input name="name_en" value="{{ old('name_en', $faculty->name_en ?? '') }}"
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <a href="{{ route('admin.academics.universities.faculties.index', $university) }}" class="rounded-xl border border-white/15 px-5 py-2.5 text-sm text-white/80 hover:bg-white/5">{{ app()->getLocale() === 'en' ? 'Cancel' : 'إلغاء' }}</a>
    </div>
</form>
